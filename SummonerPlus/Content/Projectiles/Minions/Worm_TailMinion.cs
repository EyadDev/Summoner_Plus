...
{
    public class Worm_TailMinion : ModProjectile
    {

        Projectile AttachedProjectile = new Projectile();
        Projectile Head = new Projectile();
        ...
        public override void AI()
        {
            ...
            //Projectile.rotation = AttachedProjectile.rotation;
        ...
        public void ReorderWorm()
        {
            for (int i = 0; i < Main.projectile.Length; i++)
            {
                if (Main.projectile[i].owner == Projectile.owner && Main.projectile[i].whoAmI != Projectile.whoAmI && Main.projectile[i].active && Main.projectile[i].type == ModContent.ProjectileType<Worm_BodyMinion>())
                {
            ...
            for (int i = 0; i < Main.projectile.Length; i++)
            {
                if (Main.projectile[i].owner == Projectile.owner && Main.projectile[i].whoAmI != Projectile.whoAmI && Main.projectile[i].active && Main.projectile[i].type == ModContent.ProjectileType<Worm_HeadMinion>())
                {
                    Head = Main.projectile[i];
                }
            }
        }
