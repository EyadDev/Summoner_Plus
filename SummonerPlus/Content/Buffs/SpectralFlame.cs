...
namespace SummonerPlus.Content.Buffs
{
	public class SpectralFlame : ModBuff
	{
		public override void SetStaticDefaults() {
            ...
            DisplayName.SetDefault("Spectral flame"); // Buff display name
        ...
        {
            npc.GetGlobalNPC<SpectralFlameNPC>().spectralburn = true;
            // These lines zero out any positive lifeRegen. This is expected for all bad life regeneration effects

            Dust.NewDust(npc.position, npc.width, npc.height, DustID.DungeonSpirit, 0, 0, 0, default, 1.4f);    
        }
	}

	public class SpectralFlameNPC : GlobalNPC
	{
        ...
        public override void OnHitByProjectile(NPC npc, Projectile projectile, int damage, float knockback, bool crit)
        {
            ...
            if (npc.HasBuff(ModContent.BuffType<SpectralFlame>()) && npc.type != NPCID.TargetDummy && (projectile.minion || ProjectileID.Sets.MinionShot[projectile.type]))
            {
                for (int j = 0; j < Main.npc.Length - 1; j++)
                {
                    if (Vector2.Distance(npc.position, Main.npc[j].position) < 400 && !Main.npc[j].friendly && Main.npc[j].type != NPCID.TargetDummy && !Main.npc[j].HasBuff(ModContent.BuffType<SupremeSpectralFlame>()) && !Main.npc[j].HasBuff(ModContent.BuffType<SpectralFlame>()) && !Main.npc[j].HasBuff(ModContent.BuffType<GreaterSpectralFlame>()) && !Main.npc[j].HasBuff(ModContent.BuffType<LesserSpectralFlame>()))
                    {
                        ...
                        Main.npc[j].AddBuff(ModContent.BuffType<LesserSpectralFlame>(), 60);
                        break;                        
                    }
                }
            }
